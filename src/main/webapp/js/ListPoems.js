...
class ListPoems {
  ...
  AddPoemsToPage(poemData) {
    ...
    poemData.forEach(poem => {
      let container = CreateElement('div', null, { class: 'container poem' });
      let poemTitle = CreateElement('a', poem.poemTitle, {
        'data-id': poem.id,
        href: `/poem.html?id=${poem.id}`,
        class: 'poem-title text-large'
      });
      let poemAuthor = CreateElement('p', `by ${poem.poetName}`, { class: 'poem-author text-capitalize text-small' });
